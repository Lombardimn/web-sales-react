...
const Modal = ({ title, id, className, children }: ModalComponentProps) => {
  ...
  return createPortal(
    ...
    <div className={className} ref={modalRef} onClick={handleContentClick}>
      <Button type="button" label="Cerrar" className="absolute top-2 right-2 ml-2 rounded-lg border-2 border-transparent hover:border-2 hover:outline-0 hover:border-hover-light" ParentMethod={closeModal}>
